...
def PLSR(mat_english,mat_catalan):
    pls2 = PLSRegression(n_components=80)
    pls2.fit(mat_english,mat_catalan)
    return pls2 
...
print("Precision least squares:",score/len(test_pairs))

''' Do it again with PLSR'''

score = 0
for p in test_pairs:
    en, cat = p.split()
    predicted_vector = pls2.predict(english_space[en].reshape(1,-1))[0]
